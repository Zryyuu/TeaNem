...
function confirmPayment() {
    ...
    if (currentTransactionData.type === 'ticket') {
        invoiceContent = `
            ...
            <p><strong>Pembeli:</strong> ${currentTransactionData.data.name}</p>
            <p><strong>Email:</strong> ${currentTransactionData.data.email}</p>
            <p><strong>Telepon:</strong> ${currentTransactionData.data.phone}</p>
            <br>
            <h4>Detail Pembelian:</h4>
            ${currentTransactionData.summary}
            <div class="summary-row total">
                <span><strong>Total Pembayaran:</strong></span>
                <span><strong>Rp ${currentTransactionData.total.toLocaleString('id-ID')}</strong></span>
            </div>
        `;
        
        document.getElementById('ticketDetails').innerHTML = `
            <p><strong>Nama:</strong> ${currentTransactionData.data.name}</p>
            <p><strong>Email:</strong> ${currentTransactionData.data.email}</p>
            <p><strong>No. Tiket:</strong> ${invoiceNumber}</p>
            <p><strong>Tanggal Acara:</strong> 13 Agustus 2025</p>
            <p><strong>Waktu:</strong> 07:40 - 15:00 WIB</p>
            <p><strong>Lokasi:</strong> Gedung Aula SMKN 6 Jember</p>
        `;
    } else {
        invoiceContent = `
            <p><strong>No. Invoice:</strong> ${invoiceNumber}</p>
            <p><strong>Tanggal:</strong> ${transactionDate}</p>
            <p><strong>Peserta:</strong> ${currentTransactionData.data.name}</p>
            <p><strong>Email:</strong> ${currentTransactionData.data.email}</p>
            <p><strong>Sekolah:</strong> ${currentTransactionData.data.school}</p>
            <p><strong>Usia:</strong> ${currentTransactionData.data.age} tahun</p>
            <br>
...
function printInvoice() {
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Invoice Teater Enem 2025</title>
            <style>
                body { font-family: Arial, sans-serif; padding: 20px; }
                .summary-row { display: flex; justify-content: space-between; margin: 10px 0; }
                .total { font-weight: bold; border-top: 2px solid #ff8c00; padding-top: 10px; }
                h2, h3 { color: #ff8c00; }
            </style>
        </head>
        <body>
            <h2>INVOICE - TEATER ENEM 2025</h2>
            <h3>SMKN 6 Jember</h3>
            ${invoiceContent}
            <br><br>
            <p><em>Terima kasih atas partisipasi Anda dalam Teater Enem 2025</em></p>
        </body>
        </html>
    `);
...
function printActualTicket() {
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Tiket Teater Enem 2025</title>
            <style>
                body { font-family: Arial, sans-serif; padding: 20px; text-align: center; }
                h2, h3 { color: #ff8c00; }
                .ticket-info { background: #f4f4f4; padding: 20px; margin: 20px 0; border-radius: 10px; }
            </style>
        </head>
        <body>
            <h2>TIKET TEATER ENEM 2025</h2>
            <h3>SMKN 6 Jember</h3>
            <div class="ticket-info">
                ${ticketContent}
            </div>
            <p><strong>Tunjukkan tiket ini saat masuk acara</strong></p>
            <p>13 Agustus 2025 | Gedung Aula SMKN 6 Jember</p>
        </body>
        </html>
    `);
    printWindow.document.close();
